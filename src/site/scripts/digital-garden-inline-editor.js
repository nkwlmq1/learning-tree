(function () {
  ...
  async function open(path) {
    let session;
    try { session = await request("/__api/session"); } catch (error) { window.location.href = "/__oauth/start?return=" + encodeURIComponent(location.pathname + location.search); return; }
    if (!session.authenticated) {
      window.location.href = "/__oauth/start?return=" + encodeURIComponent(location.pathname + location.search);
      return;
    }
